import { type Vec3, dot } from './vec3'
import type { Mat4 } from './mat4'

export type Quat = Float32Array

export function quat (args: number[]): Quat {
  return new Float32Array(args || [0, 0, 0, 1])
}

export function quatInvert (target: Quat, quat: Quat): Quat {
  target[0] = quat[0]
  target[1] = quat[1]
  target[2] = quat[2]
  target[3] = -quat[3]
...
export function quatNormalize (quat: Quat): Quat {
...
export function setFromAxisAngle (target: Quat, axis: Vec3, angle: number): Quat {
...
export function setFromUnitVectors (target: Quat, vFrom: Vec3, vTo: Vec3): Quat {
...
export function quatMultiply (target: Quat, quat1: Quat, quat2: Quat): Quat {
...
export function quatFromMat4 (target: Quat, mat4: Mat4): Quat {
  const te = mat4,
...
export function fromXYZW (obj: { x: number, y: number, z: number, w: number }): Quat {

...
import { addScaled, distance, subtract, vec3, vec3Lerp } from '../math/vec3'
import { saturate, smoothstep } from '../math/math'
import { getEquations } from './getEquations'
import { nextLevel, restart } from './currentLevel'
...
import { ErrorSound } from '../assets/audio/ErrorSound'
import { playVictorySound } from './playVictorySound'
import { fadeMaterial } from '../assets/materials/fadeMaterial'
import { quad } from '../assets/geometries/quad'
import { Stars } from './stars'
import { hideRestartButton, showRestartButton } from '../ui'
import type { Entity } from './types'

function pluck<T>(array: T[], condition: (item: T) => boolean) {
...
export function getLevel(entities: Entity[]) {
  ...
  const partitioner = pluck(entities, ent => ent instanceof Partitioner) as Partitioner
  let equals13Elements: SymbolElement[] = []
  let equations: SymbolElement[][]
  ...
  function checkConditionMet() {
    let partitionElements: SymbolElement[][] = partitions!.map(() => [])
  ...
  function getUsageRotation(element: SymbolElement) {

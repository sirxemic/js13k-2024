...
import { type Vec3, add, distance, dot, length, project, scale, subtract, vec3, vec3Normalize } from '../math/vec3'
import {
  canvas,
  deltaTime,
  gl,
...
import { elements, HANDLE_SIZE, levelState, STATE_PLAYING } from './shared'
import { handleMaterial } from '../assets/materials/handleMaterial'
import { quad } from '../assets/geometries/quad'
import { setPosition } from './dynamicMusic'
import { clamp } from '../math/math'
import { Spline } from './spline'
import { rotate } from '../math/vec2'
import type { Entity } from './types'

function debug(points: Vec3[]) {
...
export class Strand implements Entity {
  vertexBuffer: VertexBuffer
  startPosition: Vec3
  strandPositions: Vec3[]
  handlePosition: Vec3
  dragStart: Vec3 | undefined
  renderAlpha: number
  ...
  move(delta: Vec3, collisionCheck = true) {
  ...
  moveAndResolveCollisions(delta: Vec3, result: Vec3) {
  ...
  moveStep(delta: Vec3, collisionCheck?: boolean) {
  ...
  updatePoint(index: number, point: Vec3) {
    if (index < 0) {
  ...
  removePointsAfter(index: number) {
    if (index < 0) {
      index = this.strandPositions.length + index
    }
